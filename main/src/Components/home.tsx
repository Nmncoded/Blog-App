...
function Home():JSX.Element {
    ...
    let [articles,setArticles] = useState<null | Array<Object> >(null);
    let [allTags,setAllTags] = useState(null);
    let [offSet,setOffSet] = useState<any>(0);
    let [articlErr,setArticlErr] = useState<string>("");
    let [tagErr,setTagErr] = useState<string>("");
    ...
    useEffect(() => {
        ...
        .then(({tags}) => {
            let arr = tags.reduce((acc: string[],cv:string) => {
                if(!acc.includes(cv)){
                    acc.push(cv);
                }
                return acc;
    ...
    const handleClick = (name:any,value:any,num:any) => {
        if(value === "tags"){
            setActiveTag(name);
            setArticles(null);
        }
        ...
        if(value === "prev"){
            setOffSet(offSet - articlePerPage > 1 ? offSet - articlePerPage : 0)
        }
        if(value === "next"){
            setOffSet((offSet + articlePerPage) <= articles.length ? offSet + 10 : offSet)
        }
        if(num === "num"){
    ...
    const getPagination = (articles: Array<Object> | null ,articlePerPage:any) => {
        let arr = [];
                            ...
                            <div className='tags'>
                                <h4 className='button-shrink' >Popular Tags</h4>
                                {
                                    ...
                                    (!allTags) ? 
                                        <Loader />
                                    :
                                <Tags handleClick={(name:any) =>  handleClick(name,"tags","")} activeTag={activeTag} allTags={allTags} />
                                }
                            </div>
                        </section>
